...
if SERVER then
	...
	hook.Add("PlayerCanSeePlayersChat", Tag .. "OOC", function(text, teamOnly, listener, sender)
		if GetRoundState() == ROUND_ACTIVE
			and sender:IsSpec()
			and (sender.oocAlways
				or sender.Unrestricted
				or (ttt_allow_ooc:GetBool() and text and (text:find("OOC ", 1, true) or text:find(" OOC", 1, true))))
		then
			if sender.oocHasBody and not sender.oocMissingPass and not sender:TTT2NETGetBool("body_found", false) then
				if not sender.oocMissingWarned then
					EPOP:AddMessage(
						sender,
						{ text = "OOC message blocked!", color = Color(255, 200, 50) },
						"Your body hasn't been found yet, using OOC would reveal your death! Wait until your body gets confirmed.\n\nIf you REALLY need to use OOC now, send your message again.",
						15,
						true
					)

					sender.oocMissingWarned = true

					timer.Simple(0, function()
						-- Let any subsequent messages pass
						sender.oocMissingPass = true
					end)
				end
			...
			if not sender.oocSent then
				EPOP:AddMessage(
					sender,
					nil,
					"Remember! OOC chat can be seen by everyone. Do not abuse it to reveal traitors, or you will be punished.",
					6,
					true
				)
	...
	hook.Add("TTTPrepareRound", Tag .. "OOC", function()
		...
		for k, v in ipairs(player.GetAll()) do
			v.oocHasBody = nil
			v.oocMissingWarned = nil
			v.oocMissingPass = nil
		end
	end)
	hook.Add("TTTOnCorpseCreated", Tag .. "OOC", function(rag, pl)
		-- Flag this for the warning
		pl.oocHasBody = true
	end)
