...
if SERVER then
    ...
    function ENT:Think()
        ...
        if self.NextJumpTick and self.NextJumpTick < CurTime() then
            local radiusSqr = cvarRadius:GetFloat() ^ 2

            for _, ply in ipairs(player.GetAll()) do
                local pos = ply:GetPos()

                if ply:IsTerror() and not ply:InVehicle() and pos:DistToSqr(self:GetPos()) < radiusSqr then
    ...
    hook.Add("RenderScreenspaceEffects", "ttt_laggrenade", function()
        ...
        for id, detData in ipairs(activeGrenades) do
            if detData.expires < CurTime() then
                table.remove(activeGrenades, id)
            elseif not renderingLag and pos:DistToSqr(detData.pos) < radiusSqr then
                renderingLag = true
            ...
            if detData.nextparticle < CurTime() then
                ...
                e:SetOrigin(detData.pos + VectorRand(-radius, radius))
